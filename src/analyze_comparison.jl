...
open(latex_path, "w") do io
    ...
    # helper: wrap in \textbf if is_best flag is true
    bf1(x, best) = x == best ? @sprintf("\\textbf{%.1f}", x) : @sprintf("%.1f", x)
    bf3(x, best) = x == best ? @sprintf("\\textbf{%.3f}", x) : @sprintf("%.3f", x)
    bf2(x, best) = x == best ? @sprintf("\\textbf{%.2f}", x) : @sprintf("%.2f", x)
    ...
    for (sz, bx) in szbx_groups
        ...
        best_c   = min(c1/1e6, c2/1e6, c3/1e6)
        best_ttd = min(m1, m2, m3)

        println(io, "  $(sz_str), $(bx_str) & $n" *
            " & $(bf1(c1/1e6, best_c)) & $(bf1(m1, best_ttd))" *
            " & $(bf1(c2/1e6, best_c)) & $(bf1(m2, best_ttd))" *
            " & $(bf1(c3/1e6, best_c)) & $(bf1(m3, best_ttd))" *
            " & $adv31_str & $w31/$np \\\\")
    ...
    best_c   = min(tot_c1/1e6, tot_c2/1e6, tot_c3/1e6)
    best_ttd = min(m1, m2, m3)

    println(io, "  \\textbf{Overall} & $(length(total_1))" *
        " & $(bf1(tot_c1/1e6, best_c)) & $(bf2(m1, best_ttd))" *
        " & $(bf1(tot_c2/1e6, best_c)) & $(bf2(m2, best_ttd))" *
        " & $(bf1(tot_c3/1e6, best_c)) & $(bf2(m3, best_ttd))" *
